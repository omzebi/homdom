...
class oMySQL
{
     ...
     *
     * @uses $db = oMySQL::getInstance();
     *
     * @return oMySQL Returns the current instance.
     */
     ...
     *
     * @uses $oMySQL->setQueryOption('name');
     *
     ...
     *
     * @uses $oMySQL->where('id', 7)->where('title', 'MyTitle');
     *
     ...
     *
     * @uses $oMySQL->orWhere('id', 7)->orWhere('title', 'MyTitle');
     *
     * @param string $whereProp  The name of the database field.
     * @param mixed  $whereValue The value of the database field.
     * @param string $operator Comparison operator. Default is =
     ...
     *
     * @uses $oMySQL->having('SUM(tags) > 10')
     *
     ...
     *
     * @uses $oMySQL->orHaving('SUM(tags) > 10')
     *
     * @param string $havingProp  The name of the database field.
     * @param mixed  $havingValue The value of the database field.
     * @param string $operator Comparison operator. Default is =
     ...
     *
     * @uses $oMySQL->join('table1', 'field1 <> field2', 'LEFT')
     *
     ...
     *
     * @uses $oMySQL->orderBy('id', 'desc')->orderBy('name', 'desc', '^[a-z]')->orderBy('name', 'desc');
     *
    ...
    /**
     * Method creates new oMySQL object for a subquery generation
     * 
    ...
    /**
     * Method returns a copy of a oMySQL subquery object
     *
     * @return oMySQL new oMySQL object
     */
    ...
    /**
     * Get where and what function was called for query stored in oMySQL->trace
     *
	...
	//nombre tablas
	function Table($bd) {
		$this->_query = "SHOW TABLES FROM " . $bd;
        $stmt = $this->_buildQuery($numRows);
        $stmt->execute();
        $this->_stmtError = $stmt->error;
        $this->reset();
	...
	function Column($tableName) {
		$this->_query = "SHOW COLUMNS FROM " .self::$prefix . $tableName;
        $stmt = $this->_buildQuery($numRows);
        $stmt->execute();
        $this->_stmtError = $stmt->error;
        $this->reset();
